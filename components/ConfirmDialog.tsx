...
export function ConfirmDialog({
  ...
  confirmLabel?: string;
  busyLabel?: string;
  // "danger" = red destructive button; "default" = neutral (e.g. archive).
  tone?: "danger" | "default";
  pending?: boolean;
  onConfirm: () => void;
  onCancel: () => void;
  ...
  return (
    <dialog
      ...
      className="m-auto w-full max-w-105 rounded-[var(--radius-lg)] border border-[var(--hairline-soft)] bg-[var(--color-slate-700)] p-0 text-[var(--fg)] shadow-[0_18px_48px_rgba(20,14,10,0.46)] backdrop:bg-[rgba(8,11,14,0.66)] backdrop:backdrop-blur-[4px]"
    >
      ...
      <div className="flex justify-end gap-2.5 border-t border-[var(--hairline-soft)] px-6 pt-[18px] pb-[22px]">
        ...
        <button
          ...
          className={`${BTN} ${
            tone === "danger"
              ? "bg-[var(--color-hold-red)] text-white hover:bg-[#c23c33]"
              : "border border-[var(--hairline)] text-[var(--color-chalk-100)] hover:border-[var(--color-slate-400)] hover:bg-[var(--color-slate-600)]"
          }`}
        >
